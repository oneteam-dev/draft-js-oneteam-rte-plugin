...
var genFragment = function genFragment(node, inlineStyle, lastList, inBlock, blockTags, depth, inEntity) {
  ...
  while (child) {
    if ((0, _shouldCreateLinkEntity2.default)(nodeName, child)) {
      ...
      if (child.hasAttribute('download')) {
        entityId = _draftJs.Entity.create(_constants2.ENTITY_TYPES.DOWNLOAD_LINK, 'MUTABLE', {
          url: href,
          name: child.getAttribute('data-name'),
          size: child.getAttribute('data-size')
        });
      } else {
        entityId = _draftJs.Entity.create(_constants2.ENTITY_TYPES.LINK, 'MUTABLE', { url: href });
      }
    } else {
      entityId = undefined;
    }
...
var getChunkForHTML = function getChunkForHTML(html, DOMBuilder) {
  ...
  var chunk = genFragment(safeBody, (0, _immutable.OrderedSet)(), 'ul', null, workingBlocks, -1);

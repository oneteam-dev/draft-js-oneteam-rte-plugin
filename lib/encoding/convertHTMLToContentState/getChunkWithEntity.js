...
var getChunkWithEntity = function getChunkWithEntity(tagName, element, inlineStyle, depth) {
  if (tagName === 'img') {
    ...
    var entityKey = _DraftEntity2.default.__create(_constants2.ENTITY_TYPES.IMAGE, 'IMMUTABLE', data);
    return {
  ...
  if (tagName === 'web-card') {
    ...
    // TODO: update this when DraftEntity removed  entirely
    var _entityKey = _DraftEntity2.default.__create(_constants2.ENTITY_TYPES.WEB_CARD, 'IMMUTABLE', _data);
    return {
      text: _constants.CR + ' ' + _constants.CR,
      inlines: Array(3).fill(inlineStyle),
  ...
  if (tagName === 'file-placeholder') {
    ...
    // TODO: update this when DraftEntity removed  entirely
    var _entityKey2 = _DraftEntity2.default.__create('FILE_PLACEHOLDER', 'IMMUTABLE', _data2);
    return {
      text: _constants.CR + ' ' + _constants.CR,
      inlines: Array(3).fill(inlineStyle),
  ...
  if (tagName === 'pdf-preview' && element.parentNode.querySelectorAll('file-placeholder').length === 0) {
    ...
    // TODO: update this when DraftEntity removed  entirely
    var _entityKey3 = _DraftEntity2.default.__create('PLACEHOLDER', 'IMMUTABLE', _data3);
    return {
      text: _constants.CR + ' ' + _constants.CR,
      inlines: Array(3).fill(inlineStyle),
  ...
  if (tagName === 'iframe') {
    // TODO: update this when DraftEntity removed  entirely
    var _entityKey4 = _DraftEntity2.default.__create(_constants2.ENTITY_TYPES.IFRAME, 'IMMUTABLE', _extends({}, (0, _dom.attributesToObject)(element)));
    return {
      text: _constants.CR + ' ' + _constants.CR,
      inlines: Array(3).fill(inlineStyle),

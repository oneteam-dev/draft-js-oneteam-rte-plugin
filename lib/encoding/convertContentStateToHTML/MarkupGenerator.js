...
var MarkupGenerator = function () {
  ...
  _createClass(MarkupGenerator, [{
    ...
    value: function processBlock() {
      ...
      if ((0, _canHaveDepth2.default)(blockType) && nextBlock && nextBlock.getDepth() === block.getDepth() + 1) {
        this.output.push('\n');
    ...
    value: function writeStartTag(blockType) {
      ...
      if (blockType === _constants.OLD_BLOCK_TYPES.ALIGN_RIGHT) {
        this.output.push('<' + tag + ' style="text-align: right;">');
      } else if (blockType === _constants.OLD_BLOCK_TYPES.ALIGN_CENTER) {
        this.output.push('<' + tag + ' style="text-align: center;">');
      } else if (blockType === _constants.OLD_BLOCK_TYPES.ALIGN_JUSTIFY) {
        this.output.push('<' + tag + ' style="text-align: justify;">');
      } else if (blockType === _constants.BLOCK_TYPES.CHECKABLE_LIST_ITEM) {
